...
Usage can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.usage/config.json`
- Default: off
- UI: none

## Config File

Create `~/.usage/config.json`:
...
- Restart Usage after changing the file.
...
This applies to provider and plugin HTTP requests that go through Usage's built-in HTTP client.
