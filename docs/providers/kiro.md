...
Usage uses Kiro's local normalized usage cache first, enriches it from Kiro's own runtime logs when available, and only falls back to the live refresh/API path when the local picture is missing or stale.
...
Usage uses the latest logged response to recover plan metadata without needing network access.
