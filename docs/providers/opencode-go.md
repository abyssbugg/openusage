...
Usage reads the local OpenCode SQLite database directly:
...
Usage uses the current published OpenCode Go plan limits from the official docs:
...
Monthly usage is inferred from local history, not read from OpenCode’s account API. Usage reuses the earliest observed local OpenCode Go usage timestamp as the monthly anchor. If no local history exists yet, it falls back to UTC calendar month boundaries until the first Go usage is recorded.
...
The public provider identity stays `opencode-go`. If OpenCode later exposes account-truth usage by API key, Usage can swap the backend without changing the provider ID or UI contract.
