...
1. **Usage Keychain** (`Usage-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the Usage keychain for faster access on subsequent probes.
