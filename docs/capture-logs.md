...
Use this when Usage is not working and you need to share debug info.
...
1. In your macOS menu bar, find the Usage icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If Usage does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.datamatics.usage
```
...
1. Attach `usage.log`.
2. If you also see files like `usage.log.1`, attach those too.
