...
pub fn run_probe(plugin: &LoadedPlugin, app_data_dir: &PathBuf, app_version: &str) -> PluginOutput {
    ...
    ctx.with(|ctx| {
        ...
        let plugin_obj: Object = match globals.get("__usage_plugin") {
            Ok(obj) => obj,
            Err(_) => return error_output(plugin, "missing __usage_plugin".to_string()),
        };
...
mod tests {
    ...
    fn temp_app_dir(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("usage-test-{}-{}", label, nanos))
    }
