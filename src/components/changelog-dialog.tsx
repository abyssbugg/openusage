...
const GITHUB_REPO_URL = "https://github.com/datamatics/usage-meter"
    ...
    return parts.map((part, i) => {
      ...
      if (part.type === "pr") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`${GITHUB_REPO_URL}/pull/${part.content.slice(1)}`).catch(console.error)}
            className={linkClass}
          >
            {part.content}
          </button>
      ...
      if (part.type === "commit") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`${GITHUB_REPO_URL}/commit/${part.content}`).catch(console.error)}
              ...
              {releases.length >= 1 && (
                <div className="mt-8 pt-6 border-t border-dashed">
                  <p className="text-[10px] text-muted-foreground text-center">
                    ...
                    <button 
                      onClick={() => openUrl(`${GITHUB_REPO_URL}/releases`).catch(console.error)}
