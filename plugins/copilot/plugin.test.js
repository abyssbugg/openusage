...
function setKeychainToken(ctx, token) {
  ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
    if (service === "Usage-copilot") return JSON.stringify({ token });
    return null;
  });
}
...
describe("copilot plugin", () => {
  ...
  it("loads token from Usage keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("does not persist token loaded from Usage keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("retries with gh-cli token when cached keychain token is stale", async () => {
    ...
    expect(ctx.host.keychain.deleteGenericPassword).toHaveBeenCalledWith("Usage-copilot");
  ...
  it("falls back when Usage keychain payload lacks token field", async () => {
    const ctx = makePluginTestContext();
    ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
      if (service === "Usage-copilot") return JSON.stringify({ notToken: "x" });
