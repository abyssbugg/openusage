(function () {
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from Usage keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("Usage keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  function probe(ctx) {
    ...
    // Persist gh-cli token to Usage keychain for future use
    if (source === "gh-cli") {
      saveToken(ctx, token);
    }
  ...
  globalThis.__usage_plugin = { id: "copilot", probe };
})();
